...
namespace WpfApp1.Services
{
    public static class CarDataLoader
    {
        ...
        private static CarSale? ParseCarSale(XElement element)
        {
            ...
            bool hasPrice = double.TryParse((string?)element.Element(XmlConstants.PriceTag), out double price);
            bool hasDPH = double.TryParse((string?)element.Element(XmlConstants.VatTag), out double vat);

            if (!hasDate || !hasPrice || !hasDPH)
                return null;
